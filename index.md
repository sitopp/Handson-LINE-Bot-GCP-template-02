...
実は一度も消したことがないのですが、消し方は以下のはずです。
    
- LINE Developerにアクセス
    
https://developers.line.biz/
    
- ログイン後、左ペインの「プロバイダー」から「DEV」を選択 
- 「デジタル会員証 LINEログイン」を開く
- 画面の一番下にスクロールし、「チャネルの削除」の「削除」をクリック   
- 再度、チャネル一覧から、「デジタル会員証 Messaging API」を開く
- 画面の一番下にスクロールし、「チャネルの削除」の「削除」をクリック   
